...
def _init_layers(
  ...
  gc_nconv: Optional[int] = None) -> Tuple[nn.ModuleList, nn.ModuleList]:
  """
  ...
  """
  if num_layers > 0:
      gc_params_check = all([K, beta, gc_c_in, gc_c_out])
      tc_params_check = all([dilated_c_in, dilated_c_out, dilation_multiplier])
      skip_conv_layer_norm_check = all([dilation_multiplier, seq_len, num_nodes, kernel_size, receptive_field, skip_chans, residual_chans, dilated_c_out])
...
def _init_gc_layers(
  ...
  for _ in range(num_layers):
      layers.append(GraphConvolution(K, beta, gc_c_in, gc_c_out, nconv=gc_nconv))
  return layers
...
def _init_skip_conv_layer_norm(
  ...
  for i in range(1):
      rf_size_i = int(1 + i * (kernel_size - 1) * (dilation_multiplier ** (num_layers - 1)) / (dilation_multiplier - 1)) if dilation_multiplier > 1 else i * num_layers * (kernel_size - 1) + 1

      for j in range(1, num_layers + 1):
          rf_size_j = int(rf_size_i + (kernel_size - 1) * (dilation_multiplier ** (j - 1)) / (dilation_multiplier - 1)) if dilation_multiplier > 1 else rf_size_i + j * (kernel_size - 1)

          skip_layers.append(nn.Conv2d(in_channels=conv_chans, out_channels=skip_chans, kernel_size=(1, max(seq_len - rf_size_j + 1, receptive_field - rf_size_j + 1))))
          layer_norms.append(LayerNorm((residual_chans, num_nodes, max(seq_len - rf_size_j + 1, receptive_field - rf_size_j + 1)), elementwise_affine=True))
